...
# Requesting for the user's name
user_name = input("Enter you name: ")
print(f"Welcome {user_name}, the purpose of this Python script is to process an Excel file, extract specified fields from a JSON column, remove duplicate rows based on the 'RemotePETagID' column, and save the modified DataFrame to a new Excel file.")
...
fields_to_extract = ["DistrictID", "RemoteFarmerID", "RemotePEID", "RemotePETagID", "FarmerQuantity", "FarmerID", "RemoteSEID", "IsDeleted", "QCCNo", "WaybillNo", "SocietyID", "TagNo", "SkippedTagStatus","CategoryID", "RemoteID", "CreatedBy", "CreatedDate", "UpdatedBy", "UpdatedDate"]
for field in fields_to_extract:
    df[field] = df['JSON_Serialization'].apply(lambda x: extract_from_json(x, field))


# Remove duplicates using tag number
df.drop_duplicates(subset='RemotePETagID', keep='first', inplace=True)
